...
单文件分析完成后，Web GUI 的“方位角分析”页会对每个叶片的法向、切向和合成
载荷按 `0–360°` 默认分为 36 箱，输出每箱的样本数、均值、标准差、最小值和
最大值；同时使用一阶谐波拟合给出均值、周期幅值和相位角，并计算各叶片相对
于第一片叶片的相位差。相位结果依赖输入的方位角零点和旋转方向定义，不能替代
坐标系校核。
